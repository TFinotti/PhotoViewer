...
function addToFavorites() {
    if (favoriteCount < 5) {
        ...
        liEl.id = "item" + (favoriteCount + 1);
        var favPic = document.createElement("img");
        favPic.src = window.open("", "zoomwin").figFilename;
        favPic.width = "190";
        favPic.height = "107";
        favPic.style.padding = "10px";
        var removeButton = document.createElement("figcaption");
        removeButton.id = "photo" + (favoriteCount + 1);
        removeButton.innerHTML = "Remove";
        removeButton.style.visibility = "hidden";

        liEl.appendChild(favPic);
        liEl.appendChild(removeButton);
        ulEl.appendChild(liEl);
        window.open("", "zoomwin").close();
        favoriteCount++;

        if (favPic.addEventListener) {
            favPic.addEventListener("click", function () { showRemoveButton(removeButton.id, liEl.id); }, false);
        } else if (favPic.attachEvent) {
            favPic.attachEvent("onclick", function () { showRemoveButton(removeButton.id, liEl.id); });
        }
    }
    else {
        window.open("", "zoomwin").close();
        alert("A maximum of 5 photos can be added to your favorites. Please, remove at least one photo before adding another one.");
    }
}

function showRemoveButton(photoId, itemId) {
    var showButton = document.getElementById(photoId);
    showButton.style.visibility = "visible";
    if (showButton.addEventListener) {
        showButton.addEventListener("click", function () { removeFromFavorites(itemId); }, false);
    } else if (showButton.attachEvent) {
        showButton.attachEvent("onclick", function () { removeFromFavorites(itemId); });
    }
}
...
function zoomFig() {
    ...
    var winOptions = "width=960,height=610";
    winOptions += ",left=" + winLeft;
    winOptions += ",top=" + winTop;
    var zoomWindow = window.open("Zoom_A3.html", "zoomwin", winOptions);
    zoomWindow.focus();
}
